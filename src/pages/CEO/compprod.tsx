...
import { DatetimeChangeEventDetail, IonButton, IonCard, IonCardContent, IonContent, IonDatetime, IonHeader, IonIcon, IonInput, IonItem, IonLabel, IonModal, IonPage, IonSegment, IonSegmentButton, IonSelect, IonSelectOption, useIonLoading } from '@ionic/react';
...
const CeoCompProd: React.FC = () => {
    const user = useRecoilValue<User>(userAtom);
    const href = useRecoilValue<string>(urlAtom);
    const [present, dismiss] = useIonLoading();
    const [brands, setBrands] = useState([]);
    const [compprods, setCompProds] = useState([]);
    ...
    const handleSearch = () => {
        ...
        Http.request({
            method: "POST",
            url: href + "/api/ceo/search/compprod",
            headers: {
                'Content-Type': 'application/json',
                'Data-Type': 'json',
                'Authorization': 'Bearer ' + user.token
    ...
    //     Http.request({
    //         method: "GET",
    //         url: href + "/api/get/packaging/dashboard/" + ledger.brand_id,
    //         headers: {
    //             'Content-Type': 'application/json',
    //             'Data-Type': 'json',
    //             'Authorization': 'Bearer ' + user.token
    ...
    return (
        <IonPage>
            ...
            <IonContent className="ion-padding">
                ...
                {compprods.length > 0 && <table className='ledger-tb'>
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>Product Type</th>
                    ...
                    <tbody>
                        {compprods.map((value, index) => (<tr key={index}>
                            <td>{format(new Date(value.date), "EEEE, dd LLLL yyyy")}</td>
                            <td>{value.type}</td>
                            <td>
                                {value.sent}
                                {value.sent !== value.received && <IonIcon icon={value.sent > value.received ? chevronUp : chevronDown} color={value.sent > value.received ? "success" : "danger"} />}
                            </td>
                            <td>
                                {value.received}
                                {value.sent !== value.received && <IonIcon icon={value.received > value.sent ? chevronUp : chevronDown} color={value.received > value.sent ? "success" : "danger"} />}
                            </td>
                            {/* <td className='!p-0'>
